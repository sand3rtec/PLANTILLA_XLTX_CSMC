--================================================================
--  Ficha Seguimiento · TRATAMIENTO AMBULATORIO DE NIÑOS Y NIÑAS DE 0 A 17 AÑOS TRASTORNO_AUTISTA (F84.0, F84.1, F84.5 F84.8 F84.9)
--  Base: BDHIS_MINSA  |  Destino: dbo.ficha_TRASTORNO_AUTISTA_csmc25
...
IF OBJECT_ID('dbo.ficha_TRASTORNO_AUTISTA_csmc25', 'U') IS NOT NULL
    DROP TABLE dbo.ficha_TRASTORNO_AUTISTA_csmc25;
...
-- 2) Primer diagnóstico 'F840','F841','F845','F848','F849' por paciente
Dx AS (
    SELECT *, ROW_NUMBER() OVER (PARTITION BY DNI ORDER BY Fecha_Atencion) AS RankDx
    FROM   Base
    WHERE  Tipo_Diagnostico = 'D'
      AND  (Codigo_Item IN ('F840','F841','F845','F848','F849'))
),
...
Act AS (
    SELECT B.DNI,
        ...
        MAX(CASE WHEN B.Codigo_Item IN ('90806','90834','90860') AND B.Tipo_Diagnostico = 'D' AND B.Id_Correlativo_Item IN (2,3,4,5,6) AND B.Valor_Lab = '6' THEN B.Fecha_Atencion END) AS Fec_PS6,
		 ...
		 -- intervención grupal IG (sesión 1-6)
        MAX(CASE WHEN B.Codigo_Item IN ('99207.02','Z507','97009') AND B.Tipo_Diagnostico = 'D' AND B.Id_Correlativo_Item IN (2,3,4,5,6) AND B.Valor_Lab = '1' THEN B.Fecha_Atencion END) AS Fec_IG1,
		MAX(CASE WHEN B.Codigo_Item IN ('99207.02','Z507','97009') AND B.Tipo_Diagnostico = 'D' AND B.Id_Correlativo_Item IN (2,3,4,5,6) AND B.Valor_Lab = '2' THEN B.Fecha_Atencion END) AS Fec_IG2,
		MAX(CASE WHEN B.Codigo_Item IN ('99207.02','Z507','97009') AND B.Tipo_Diagnostico = 'D' AND B.Id_Correlativo_Item IN (2,3,4,5,6) AND B.Valor_Lab = '3' THEN B.Fecha_Atencion END) AS Fec_IG3,
...
SELECT
	...
	A.Fec_IG1,A.Fec_IG2,A.Fec_IG3,A.Fec_IG4,A.Fec_IG5,A.Fec_IG6,
	A.Fec_VI1

INTO  dbo.ficha_TRASTORNO_AUTISTA_csmc25
FROM  Dx D
LEFT JOIN Act     A  ON D.DNI = A.DNI
LEFT JOIN Acogida AC ON D.DNI = AC.DNI
WHERE D.RankDx = 1;

SELECT * FROM dbo.ficha_TRASTORNO_AUTISTA_csmc25
