-- ================================================================
--  Ficha Seguimiento · Tratamiento ambulatorio a personas con síndrome psicótico o 
-- trastorno del espectro de la esquizofrenia (F20 al F29, F31.2, F31.5, F32.3, F33.3, F06.2, F10.5, F11.5, F12.5, 
-- F13.5, F14.5, F15.5, F16.5, F17.5, F18.5 Y F19.5)
--  Base: BDHIS_MINSA  |  Destino: dbo.ficha_ESQUIZOFRENIA_csmc25
--  Período: 2025
-- ================================================================
...
IF OBJECT_ID('dbo.ficha_ESQUIZOFRENIA_csmc25', 'U') IS NOT NULL
    DROP TABLE dbo.ficha_ESQUIZOFRENIA_csmc25;
...
Act AS (
    SELECT B.DNI,
        ...
        -- intervención familiar IF (sesión 1-2)
       ...
       -- MAX(CASE WHEN B.Codigo_Item IN ('90806','90834','90860') AND B.Tipo_Diagnostico = 'D' AND B.Id_Correlativo_Item IN (2,3,4,5,6) AND B.Valor_Lab = '5' THEN B.Fecha_Atencion END) AS Fec_PS5,
		...
		MAX(CASE WHEN B.Codigo_Item = '99207.04' AND B.Tipo_Diagnostico = 'D' AND B.Id_Correlativo_Item IN (2,3,4,5,6) AND B.Valor_Lab = '2' THEN B.Fecha_Atencion END) AS Fec_EDU2,
		MAX(CASE WHEN B.Codigo_Item = '99207.04' AND B.Tipo_Diagnostico = 'D' AND B.Id_Correlativo_Item IN (2,3,4,5,6) AND B.Valor_Lab = '3' THEN B.Fecha_Atencion END) AS Fec_EDU3,
		MAX(CASE WHEN B.Codigo_Item = '99207.04' AND B.Tipo_Diagnostico = 'D' AND B.Id_Correlativo_Item IN (2,3,4,5,6) AND B.Valor_Lab = '4' THEN B.Fecha_Atencion END) AS Fec_EDU4,
...
INTO  dbo.ficha_ESQUIZOFRENIA_csmc25
FROM  Dx D
LEFT JOIN Act     A  ON D.DNI = A.DNI
LEFT JOIN Acogida AC ON D.DNI = AC.DNI
WHERE D.RankDx = 1;

SELECT * FROM dbo.ficha_ESQUIZOFRENIA_csmc25
