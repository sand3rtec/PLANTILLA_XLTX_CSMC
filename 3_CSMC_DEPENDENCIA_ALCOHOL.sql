-- ================================================================
--  Ficha Seguimiento · Intervención para personas con dependencia del alcohol y tabaco (F10.2 y F17.2)
--  Base: BDHIS_MINSA  |  Destino: dbo.ficha_DEPENDENCIA_ALCOHOL_csmc25
...
IF OBJECT_ID('dbo.ficha_DEPENDENCIA_ALCOHOL_csmc25', 'U') IS NOT NULL
    DROP TABLE dbo.ficha_DEPENDENCIA_ALCOHOL_csmc25;
...
Act AS (
    SELECT B.DNI,
	   ...
	   MAX(CASE WHEN B.Codigo_Item = '99215' AND B.Tipo_Diagnostico = 'D' AND B.Id_Correlativo_Item IN (2,3,4,5,6) AND B.Valor_Lab = '4' THEN B.Fecha_Atencion END) AS Fec_CS4,
        ...
        -- entrevistas motivacionales EM (sesión 1-2)
        MAX(CASE WHEN B.Codigo_Item = '96150' AND B.Tipo_Diagnostico = 'D' AND B.Id_Correlativo_Item IN (2,3,4,5,6) AND B.Valor_Lab = '1' THEN B.Fecha_Atencion END) AS Fec_EM1,
		MAX(CASE WHEN B.Codigo_Item = '96150' AND B.Tipo_Diagnostico = 'D' AND B.Id_Correlativo_Item IN (2,3,4,5,6) AND B.Valor_Lab = '2' THEN B.Fecha_Atencion END) AS Fec_EM2,

        -- psicoterapias individuales PS (sesiones 1-4)
...
SELECT
    ...
    A.Fec_EM1,A.Fec_EM2,
    A.Fec_PS1, A.Fec_PS2, A.Fec_PS3, A.Fec_PS4,
	A.Fec_IF1,A.Fec_IF2

INTO  dbo.ficha_DEPENDENCIA_ALCOHOL_csmc25
FROM  Dx D
LEFT JOIN Act     A  ON D.DNI = A.DNI
LEFT JOIN Acogida AC ON D.DNI = AC.DNI
WHERE D.RankDx = 1;

SELECT * FROM dbo.ficha_DEPENDENCIA_ALCOHOL_csmc25
