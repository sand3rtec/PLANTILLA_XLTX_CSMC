-- ================================================================
--  Ficha Seguimiento · VIOLENCIA_FAMILIAR (T740, T741, T743, T748, T749,Y04, Y06, Y07, Y08)
--  Base: BDHIS_MINSA  |  Destino: dbo.ficha_VIOLENCIA_FAMILIAR_csmc25
...
IF OBJECT_ID('dbo.ficha_VIOLENCIA_FAMILIAR_csmc25', 'U') IS NOT NULL
    DROP TABLE dbo.ficha_VIOLENCIA_FAMILIAR_csmc25;
...
-- 2) Primer diagnóstico 'Y070','T741','T743','T748','T749','Y040','Y080','Y060' por paciente
Dx AS (
    SELECT *, ROW_NUMBER() OVER (PARTITION BY DNI ORDER BY Fecha_Atencion) AS RankDx
    FROM   Base
    WHERE  Tipo_Diagnostico = 'D'
      AND  (Codigo_Item IN ('Y070','T740','T741','T743','T748','T749','Y040','Y08','Y060') and Codigo_Item !='T742')
),
...
Act AS (
    SELECT B.DNI,
        ...
        MAX(CASE WHEN B.Codigo_Item IN ('90806','90834','90860') AND B.Tipo_Diagnostico = 'D' AND B.Id_Correlativo_Item IN (2,3,4,5,6) AND B.Valor_Lab = '6' THEN B.Fecha_Atencion END) AS Fec_PS6,
		 ...
		 -- intervención familiar IF (sesión 1)
...
INTO  dbo.ficha_VIOLENCIA_FAMILIAR_csmc25
FROM  Dx D
LEFT JOIN Act     A  ON D.DNI = A.DNI
LEFT JOIN Acogida AC ON D.DNI = AC.DNI
WHERE D.RankDx = 1;

SELECT * FROM dbo.ficha_VIOLENCIA_FAMILIAR_csmc25
