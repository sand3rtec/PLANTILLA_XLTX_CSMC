-- ================================================================
--  Ficha Seguimiento · Intervenciones breves motivacionales para personas con consumo perjudicial del alcohol y tabaco (F10.1 y F17.1)
--  Base: BDHIS_MINSA  |  Destino: dbo.ficha_INTERV_BREVES_csmc25
...
IF OBJECT_ID('dbo.ficha_INTERV_BREVES_csmc25', 'U') IS NOT NULL
    DROP TABLE dbo.ficha_INTERV_BREVES_csmc25;
...
Act AS (
    SELECT B.DNI,
       ...
       -- MAX(CASE WHEN B.Codigo_Item = '99215' AND B.Tipo_Diagnostico = 'D' AND B.Id_Correlativo_Item IN (2,3,4,5,6) AND B.Valor_Lab = '3' THEN B.Fecha_Atencion END) AS Fec_CS3,
        ...
        -- consejería en estilos de vida saludable CON (sesión 1)
        MAX(CASE WHEN B.Codigo_Item = '99401.13' AND B.Tipo_Diagnostico = 'D' AND B.Id_Correlativo_Item IN (2,3,4,5,6) AND B.Valor_Lab = '1' THEN B.Fecha_Atencion END) AS Fec_CON1,

        -- intervenciones breves IB (sesiones 1-4)
        MAX(CASE WHEN B.Codigo_Item IN ('99207.01') AND B.Tipo_Diagnostico = 'D' AND B.Id_Correlativo_Item IN (2,3,4,5,6) AND B.Valor_Lab = '1' THEN B.Fecha_Atencion END) AS Fec_IB1,
        MAX(CASE WHEN B.Codigo_Item IN ('99207.01') AND B.Tipo_Diagnostico = 'D' AND B.Id_Correlativo_Item IN (2,3,4,5,6) AND B.Valor_Lab = '2' THEN B.Fecha_Atencion END) AS Fec_IB2,
        MAX(CASE WHEN B.Codigo_Item IN ('99207.01') AND B.Tipo_Diagnostico = 'D' AND B.Id_Correlativo_Item IN (2,3,4,5,6) AND B.Valor_Lab = '3' THEN B.Fecha_Atencion END) AS Fec_IB3,
        MAX(CASE WHEN B.Codigo_Item IN ('99207.01') AND B.Tipo_Diagnostico = 'D' AND B.Id_Correlativo_Item IN (2,3,4,5,6) AND B.Valor_Lab = '4' THEN B.Fecha_Atencion END) AS Fec_IB4
...
SELECT
    ...
    A.PAI,
    --A.Fec_CS1, A.Fec_CS2, A.Fec_CS3,
    A.Fec_CON1,
    A.Fec_IB1, A.Fec_IB2, A.Fec_IB3, A.Fec_IB4

INTO  dbo.ficha_INTERV_BREVES_csmc25
FROM  Dx D
LEFT JOIN Act     A  ON D.DNI = A.DNI
LEFT JOIN Acogida AC ON D.DNI = AC.DNI
WHERE D.RankDx = 1;

SELECT * FROM dbo.ficha_INTERV_BREVES_csmc25
