-- ================================================================
--  Ficha Seguimiento · DEPRESIÓN (F31-F34, F38)
--  Base: BDHIS_MINSA  |  Destino: dbo.ficha_depresion_csmc25
...
IF OBJECT_ID('dbo.ficha_depresion_csmc25', 'U') IS NOT NULL
    DROP TABLE dbo.ficha_depresion_csmc25;
...
Dx AS (
    ...
    WHERE  Tipo_Diagnostico = 'D'
      AND  (Codigo_Item BETWEEN 'F31' AND 'F34' OR Codigo_Item = 'F38')
),
...
INTO  dbo.ficha_depresion_csmc25
FROM  Dx D
LEFT JOIN Act     A  ON D.DNI = A.DNI
LEFT JOIN Acogida AC ON D.DNI = AC.DNI
WHERE D.RankDx = 1;

SELECT * FROM dbo.ficha_depresion_csmc25
