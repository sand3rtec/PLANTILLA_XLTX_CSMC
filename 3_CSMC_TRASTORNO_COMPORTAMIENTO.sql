--================================================================
--  Ficha Seguimiento · TRATAMIENTO AMBULATORIO DE NIÑOS Y NIÑAS DE 0 A 17 AÑOS TRASTORNO_COMPORTAMIENTO (F50-F59, F80-F83, F90-F98)
--  Base: BDHIS_MINSA  |  Destino: dbo.ficha_TRASTORNO_COMPORTAMIENTO_csmc25
--  Período: 2025 < 18 AÑOS
-- ================================================================
...
IF OBJECT_ID('dbo.ficha_TRASTORNO_COMPORTAMIENTO_csmc25', 'U') IS NOT NULL
    DROP TABLE dbo.ficha_TRASTORNO_COMPORTAMIENTO_csmc25;
...
Base AS (
    ...
    WHERE h.Anio = '2025'and Edad_Reg <'18'
...
-- 2) Primer diagnóstico F50-F59, F80-F83, F90-F98 por paciente
Dx AS (
    SELECT *, ROW_NUMBER() OVER (PARTITION BY DNI ORDER BY Fecha_Atencion) AS RankDx
    FROM   Base
    WHERE  Tipo_Diagnostico = 'D'
      AND  (Codigo_Item BETWEEN 'F50'AND'F59' OR (Codigo_Item BETWEEN 'F80'AND'F83')OR (Codigo_Item BETWEEN 'F90'AND'F98'))
),
...
Act AS (
    SELECT B.DNI,
        ...
        MAX(CASE WHEN B.Codigo_Item = '99366'
                  ...
                  AND B.Id_Correlativo_Item IN (2,3,4,5,6)
                  AND B.Valor_Lab BETWEEN '1' AND '6'
                 THEN '1' ELSE '0' END)                     AS PAI,

        -- Consulta Médica CS (sesiones 1-3)
        MAX(CASE WHEN B.Codigo_Item IN ('99207','99215') AND B.Tipo_Diagnostico = 'D' AND B.Id_Correlativo_Item IN (2,3,4,5,6) AND B.Valor_Lab = '1' THEN B.Fecha_Atencion END) AS Fec_CS1,
        MAX(CASE WHEN B.Codigo_Item IN ('99207','99215') AND B.Tipo_Diagnostico = 'D' AND B.Id_Correlativo_Item IN (2,3,4,5,6) AND B.Valor_Lab = '2' THEN B.Fecha_Atencion END) AS Fec_CS2,
        ...
        MAX(CASE WHEN B.Codigo_Item IN ('90806','90834','90860') AND B.Tipo_Diagnostico = 'D' AND B.Id_Correlativo_Item IN (2,3,4,5,6) AND B.Valor_Lab = '6' THEN B.Fecha_Atencion END) AS Fec_PS6,
		 ...
		 -- intervención Familiar IG (sesión 1-3)
        MAX(CASE WHEN B.Codigo_Item IN ('C2111.01','96100.01','90847') AND B.Tipo_Diagnostico = 'D' AND B.Id_Correlativo_Item IN (2,3,4,5,6) AND B.Valor_Lab = '1' THEN B.Fecha_Atencion END) AS Fec_IF1,
		MAX(CASE WHEN B.Codigo_Item IN ('C2111.01','96100.01','90847') AND B.Tipo_Diagnostico = 'D' AND B.Id_Correlativo_Item IN (2,3,4,5,6) AND B.Valor_Lab = '2' THEN B.Fecha_Atencion END) AS Fec_IF2,
		MAX(CASE WHEN B.Codigo_Item IN ('C2111.01','96100.01','90847') AND B.Tipo_Diagnostico = 'D' AND B.Id_Correlativo_Item IN (2,3,4,5,6) AND B.Valor_Lab = '3' THEN B.Fecha_Atencion END) AS Fec_IF3,
		--MAX(CASE WHEN B.Codigo_Item IN ('99207.02','Z507','97009') AND B.Tipo_Diagnostico = 'D' AND B.Id_Correlativo_Item IN (2,3,4,5,6) AND B.Valor_Lab = '4' THEN B.Fecha_Atencion END) AS Fec_IG4,
		--MAX(CASE WHEN B.Codigo_Item IN ('99207.02','Z507','97009') AND B.Tipo_Diagnostico = 'D' AND B.Id_Correlativo_Item IN (2,3,4,5,6) AND B.Valor_Lab = '5' THEN B.Fecha_Atencion END) AS Fec_IG5,
		--MAX(CASE WHEN B.Codigo_Item IN ('99207.02','Z507','97009') AND B.Tipo_Diagnostico = 'D' AND B.Id_Correlativo_Item IN (2,3,4,5,6) AND B.Valor_Lab = '6' THEN B.Fecha_Atencion END) AS Fec_IG6,

		-- Visita domiciliara o 01 movilización de redes de apoyo (VI)
...
SELECT
	...
	A.Fec_IF1,A.Fec_IF2,A.Fec_IF3,
	A.Fec_VI1

INTO  dbo.ficha_TRASTORNO_COMPORTAMIENTO_csmc25
FROM  Dx D
LEFT JOIN Act     A  ON D.DNI = A.DNI
LEFT JOIN Acogida AC ON D.DNI = AC.DNI
WHERE D.RankDx = 1;

SELECT * FROM dbo.ficha_TRASTORNO_COMPORTAMIENTO_csmc25
